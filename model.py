...
class MaskedKLDivLoss(nn.Module):
    ...
    def forward(self, log_pred, target, mask):
        mask_ = mask.view(-1, 1)
        # Use the fused prediction as a fixed teacher distribution.
        loss = self.loss(log_pred * mask_, target.detach() * mask_) / torch.sum(mask)
        return loss
    ...
    def forward(self, textf, visuf, acouf, u_mask, qmask, dia_len):
        ...
        if self.n_speakers == 2:
            for i, x in enumerate(dia_len):
                spk_idx[i, x:] = torch.full((origin_spk_idx[i].size(0)-x,), 2,
                                            dtype=spk_idx.dtype, device=spk_idx.device)
        if self.n_speakers == 9:
            for i, x in enumerate(dia_len):
                spk_idx[i, x:] = torch.full((origin_spk_idx[i].size(0)-x,), 9,
                                            dtype=spk_idx.dtype, device=spk_idx.device)
